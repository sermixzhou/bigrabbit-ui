...
export function Input({ label, helper, errorMessage, clearable = true, onClear, className, id: providedId, value, disabled, ...props }: InputProps) {
  ...
  return (
    <div className="block w-full">
      ...
      <span className={cn("flex h-12 items-center gap-2 rounded-cb-sm border bg-white pl-4 transition focus-within:border-brand focus-within:bg-[#F2F7FF] focus-within:ring-4 focus-within:ring-[rgba(10,124,255,.1)]", errorMessage ? "border-danger bg-[#FFF1F0]" : "border-line", disabled && "border-[#D4DBE6] bg-[#F5F7FA]", className)}>
...
export function Search({ loading, value, onClear, className, ...props }: SearchProps) {
  const hasValue = String(value ?? "").length > 0;
  return (
    <label className={cn("flex h-12 w-full items-center gap-2 rounded-cb-sm border border-line bg-white px-4 transition focus-within:border-brand focus-within:bg-[#F2F7FF] focus-within:ring-4 focus-within:ring-[rgba(10,124,255,.1)]", props.disabled && "bg-[#F5F7FA]", className)}>
      ...
      {loading ? <span className="size-4 animate-spin rounded-full border-2 border-brand border-r-transparent" /> : hasValue && !props.disabled ? <button type="button" aria-label="清空搜索" className="cb-focus -mr-2 flex size-10 items-center justify-center rounded-full text-[#8A96A8]" onClick={onClear}><Icon name="close" size={20} /></button> : null}
    </label>
  );
}
