...
export function DifferenceBlock({ firstWord, firstDescription, secondWord, secondDescription }: { firstWord: string; firstDescription: string; secondWord: string; secondDescription: string }) {
  return <div className="grid gap-3 rounded-br border border-[#CFE4FF] bg-[#F2F7FF] p-4 sm:grid-cols-2"><div className="rounded-br-sm bg-white p-3"><p className="text-lg font-bold text-brand-deep">{firstWord}</p><p className="mt-1 text-sm text-muted">{firstDescription}</p></div><div className="rounded-br-sm bg-white p-3"><p className="text-lg font-bold text-brand-deep">{secondWord}</p><p className="mt-1 text-sm text-muted">{secondDescription}</p></div></div>;
}
...
export function CollocationItem({ phrase, translation, selected, onClick }: { phrase: string; translation?: string; selected?: boolean; onClick?: () => void }) {
  return <button onClick={onClick} aria-pressed={selected} className={cn("br-control flex min-h-11 w-full items-center justify-between gap-3 rounded-br-sm border border-line bg-white px-3 text-left", selected && "border-brand bg-[#F2F7FF]")}><span><span className="font-medium text-brand-deep">{phrase}</span>{translation && <span className="ml-2 text-sm text-muted">{translation}</span>}</span>{selected && <Icon name="check" size={20} className="text-brand" />}</button>;
}
