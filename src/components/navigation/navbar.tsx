...
export function Navbar({ brand, items = [], action, open, defaultOpen = false, onOpenChange, ariaLabel, openMenuLabel, closeMenuLabel, className }: NavbarProps) {
  ...
  return <header className={cn("border-b border-border bg-surface", className)}><nav aria-label={ariaLabel ?? messages.mainNavigation} className="mx-auto max-w-screen-xl px-4 sm:px-6"><div className="flex min-h-16 items-center justify-between gap-4"><div className="shrink-0">{brand}</div><div className="hidden items-center gap-1 md:flex">{links}</div><div className="hidden shrink-0 md:block">{action}</div><button type="button" aria-label={expanded ? closeMenuLabel ?? messages.closeNavigationMenu : openMenuLabel ?? messages.openNavigationMenu} aria-expanded={expanded} onClick={() => update(!expanded)} className="cb-focus flex size-11 items-center justify-center rounded-cb-sm text-xl text-text-strong hover:bg-primary-subtle md:hidden"><span aria-hidden="true">{expanded ? "×" : "☰"}</span></button></div>{expanded && <div className="border-t border-border py-3 md:hidden"><div className="flex flex-col gap-1">{links}</div>{action && <div className="mt-3 border-t border-border pt-3">{action}</div>}</div>}</nav></header>;
}
