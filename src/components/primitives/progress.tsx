...
export function Progress({ value, max = 100, size = "standard", state = "default", label, showValue = false, className }: ProgressProps) {
  ...
  const fill = state === "success" ? "bg-success" : state === "error" ? "bg-danger" : "bg-primary";
  return <div className={cn("w-full", className)}>{(label || showValue) && <div className="mb-2 flex items-center justify-between text-xs text-text-muted"><span>{label}</span>{showValue && <span>{Math.round(percent)}%</span>}</div>}<div className={cn("overflow-hidden rounded-full bg-primary-soft", size === "slim" ? "h-1" : "h-2")} role="progressbar" aria-valuemin={0} aria-valuemax={max} aria-valuenow={value}><div className={cn("h-full rounded-full transition-[width] duration-300", fill)} style={{ width: `${percent}%` }} /></div></div>;
}
