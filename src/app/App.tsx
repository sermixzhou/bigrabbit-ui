...
function Showcase({ title, purpose, children, className, code, note }: { title: string; purpose: string; children: ReactNode; className?: string; code?: string; note?: string }) {
  return <article className={cn("overflow-hidden rounded-cb border border-line bg-white", className)}><header className="border-b border-line px-4 py-3.5 sm:px-5"><h3 className="text-[17px] font-semibold text-brand-deep">{title}</h3><p className="mt-0.5 text-sm text-muted">{purpose}</p></header><div className="bg-[radial-gradient(#CFE4FF_0.7px,transparent_0.7px)] bg-[length:14px_14px] p-4 sm:p-5"><div className="rounded-cb-sm bg-white/80 p-1">{children}</div></div>{note && <p className="border-t border-line bg-[#FAFBFC] px-4 py-3 text-xs leading-relaxed text-muted"><strong className="text-brand-deep">使用说明：</strong>{note}</p>}{code && <details className="border-t border-line"><summary className="cb-focus cursor-pointer list-none px-4 py-3 text-sm font-semibold text-brand hover:bg-[#F2F7FF]">查看示例代码</summary><pre className="cb-code m-4 mt-0"><code>{code}</code></pre></details>}</article>;
}

function Guidelines({ doText, dontText }: { doText: string; dontText: string }) {
  return <div className="mt-5 grid gap-3 sm:grid-cols-2"><div className="rounded-cb-sm bg-[#ECFDF3] p-4 text-sm leading-relaxed text-[#166534]"><strong className="mb-1 flex items-center gap-1"><Icon name="check" size={16} />推荐</strong>{doText}</div><div className="rounded-cb-sm bg-[#FFF1F0] p-4 text-sm leading-relaxed text-[#B42318]"><strong className="mb-1 flex items-center gap-1"><Icon name="close" size={16} />避免</strong>{dontText}</div></div>;
}
...
function Lists() {
  return <div className="grid gap-5 xl:grid-cols-2"><Showcase title="List Item" purpose="用扁平列表清晰呈现可扫描内容" note="单行高度 56px，双行高度 64px；同组列表共享一个外层边框。"><div className="overflow-hidden rounded-cb border border-line"><ListItem leadingIcon="book" title="课程内容" description="查看全部学习单元" trailing="12 单元" /><ListItem leadingIcon="calendar" title="学习计划" description="每天 10 个新词" state="selected" /><ListItem leadingIcon="achievement" title="已完成任务" state="completed" /><ListItem leadingIcon="lock" title="高级挑战" state="locked" /></div></Showcase><Showcase title="Word List" purpose="统一承载序号、单词、释义和学习状态"><div className="overflow-hidden rounded-cb border border-line"><WordListItem index={1} word="family" meaning="n. 家庭" /><WordListItem index={2} word="father" meaning="n. 父亲；爸爸" state="current" status="学习中" /><WordListItem index={3} word="mother" meaning="n. 母亲；妈妈" state="completed" status="已掌握" /><WordListItem index={4} word="brother" meaning="n. 兄弟；哥哥" state="locked" /></div></Showcase><Showcase title="Settings List" purpose="设置项使用相同的信息结构" className="xl:col-span-2"><div className="mx-auto max-w-2xl overflow-hidden rounded-cb border border-line"><SettingListItem icon="review" title="学习报告" description="查看本周的学习表现" /><SettingListItem icon="settings" title="学习设置" value="每天 10 词" /><SettingListItem icon="info" title="关于我们" value="v0.1.0" /></div></Showcase></div>;
}

function Navigation() {
  const [tab, setTab] = useState("home");
  const [lineTab, setLineTab] = useState("all");
  return <div className="grid gap-5 xl:grid-cols-2"><Showcase title="Top Navigation" purpose="保持页面位置和退出路径清晰"><div className="overflow-hidden rounded-cb border border-line"><TopNavigation title="单词学习" subtitle="2 / 5" actionIcon="more" /><TopNavigation title="我的成就" back={false} actionIcon="share" align="left" /></div></Showcase><Showcase title="Tabs" purpose="在同一页面内切换同层级内容"><div className="space-y-5"><Tabs items={[{ label: "全部", value: "all" }, { label: "未学习", value: "new" }, { label: "已掌握", value: "done" }]} value={lineTab} onChange={setLineTab} /><Tabs variant="pill" items={[{ label: "单词", value: "words" }, { label: "短语", value: "phrases" }]} value={lineTab === "words" ? "words" : "phrases"} onChange={setLineTab} /></div></Showcase><Showcase title="Bottom Tab Bar" purpose="最多承载 4–5 个顶层目的地" className="xl:col-span-2" note="移动端固定在安全区上方；桌面端通常由侧栏导航替代。"><div className="mx-auto max-w-xl overflow-hidden rounded-cb border border-line pt-16"><BottomTabBar items={tabItems} value={tab} onChange={setTab} /></div></Showcase></div>;
}
